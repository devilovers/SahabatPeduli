...
<div class="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
    <div class="max-w-md w-full space-y-8 bg-white dark:bg-slate-900 p-8 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl transition-colors">
        <div class="text-center space-y-2">
            ...
            <h2 class="text-xl font-bold text-slate-900 dark:text-white pt-2">Buat Akun</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Bergabung bersama kami untuk kebaikan bersama</p>
        </div>

        <?php if (!empty($errors)): ?>
            <div class="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-2xl text-xs space-y-1">
        ...
        <?php if ($success): ?>
            <div class="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800 text-brand-800 dark:text-brand-300 px-4 py-3 rounded-2xl text-xs flex justify-between items-center">
                <span><i class="fa-solid fa-circle-check mr-1"></i> <?= htmlspecialchars($success); ?></span>
                <a href="login.php" class="font-bold underline text-brand-600 dark:text-brand-400">Masuk</a>
            </div>
        <?php endif; ?>
        ...
        <form method="POST" action="" class="space-y-4">
            <div>
                ...
                <input type="text" name="name" required class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none transition-colors" placeholder="Masukkan nama lengkap">
            </div>
            ...
            <div>
                ...
                <input type="text" name="phone" class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none transition-colors" placeholder="08xxxxxxxxxx">
            </div>

            <div>
                <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">Password</label>
                <input type="password" name="password" required class="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm focus:ring-2 focus:ring-brand-500 focus:outline-none transition-colors" placeholder="Minimal 6 karakter">
            </div>
        ...
        <p class="text-center text-xs text-slate-500 dark:text-slate-400">
            Sudah punya akun? <a href="login.php" class="text-brand-600 dark:text-brand-400 font-bold hover:underline">Masuk di sini</a>
        </p>
    </div>
</div>
