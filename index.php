...
$stmtArticles = $pdo->query("SELECT articles.*, users.name as author_name FROM articles JOIN users ON articles.author_id = users.id ORDER BY articles.created_at DESC LIMIT 3");
$articles = $stmtArticles->fetchAll();

$stmtDistributions = $pdo->query("SELECT id, title, amount_spent, latitude, longitude, location_name, distribution_date FROM distributions ORDER BY distribution_date DESC");
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-7 space-y-6 text-center lg:text-left">
                <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-slate-900 dark:text-white tracking-tight leading-tight">
                    Penyaluran ZISWAF <br class="hidden sm:inline" />
                ...
                <p class="text-base sm:text-lg text-slate-600 dark:text-slate-300 max-w-2xl mx-auto lg:mx-0 font-normal leading-relaxed">
                    PeduliUmat membantu menyalurkan Zakat, Infaq, Sedekah, Wakaf, dan Fidyah Anda langsung kepada Mustahik secara akuntabel dan terpeta secara real-time.
                </p>

                <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-2">
                    <a href="/PeduliUmat/views/donasi.php" class="w-full sm:w-auto px-8 py-4 rounded-2xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-base shadow-lg shadow-brand-600/30 hover:shadow-xl transition-all flex items-center justify-center gap-3 group">
                        <span>Tunaikan Donasi</span>
                        <i class="fa-solid fa-arrow-right group-hover:translate-x-1 transition-transform"></i>
                    </a>
                    <a href="/PeduliUmat/views/kalkulator.php" class="w-full sm:w-auto px-8 py-4 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-brand-500 text-slate-700 dark:text-slate-200 hover:text-brand-600 dark:hover:text-brand-400 font-bold text-base shadow-sm hover:shadow-md transition-all flex items-center justify-center gap-2">
                ...
                <div class="relative mx-auto w-full max-w-md bg-white/80 dark:bg-slate-800/80 backdrop-blur-xl border border-white dark:border-slate-700/50 p-6 sm:p-8 rounded-3xl shadow-2xl shadow-emerald-900/10 dark:shadow-black/40 space-y-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                <h3 class="font-bold text-slate-900 dark:text-white">Kategori ZISWAF</h3>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <a href="/PeduliUmat/views/donasi.php?type=zakat" class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 hover:bg-brand-50 dark:hover:bg-slate-700/80 border border-slate-100 dark:border-slate-700 hover:border-brand-200 dark:hover:border-slate-600 transition-all text-center group">
                        ...
                        <a href="/PeduliUmat/views/donasi.php?type=infaq" class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 hover:bg-brand-50 dark:hover:bg-slate-700/80 border border-slate-100 dark:border-slate-700 hover:border-brand-200 dark:hover:border-slate-600 transition-all text-center group">
                        ...
                        <a href="/PeduliUmat/views/donasi.php?type=sedekah" class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 hover:bg-brand-50 dark:hover:bg-slate-700/80 border border-slate-100 dark:border-slate-700 hover:border-brand-200 dark:hover:border-slate-600 transition-all text-center group">
                        ...
                        <a href="/PeduliUmat/views/donasi.php?type=wakaf" class="p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/60 hover:bg-brand-50 dark:hover:bg-slate-700/80 border border-slate-100 dark:border-slate-700 hover:border-brand-200 dark:hover:border-slate-600 transition-all text-center group">
                            ...
                            <span class="font-bold text-sm text-slate-800 dark:text-slate-200 group-hover:text-brand-700 dark:group-hover:text-brand-400 block">Wakaf & Fidyah</span>
                            <span class="text-[10px] text-brand-600 dark:text-brand-400 font-medium block italic">(وَقْف) Menahan & (فِدْيَة) Teburan</span>
                            <p class="text-[11px] text-slate-500 dark:text-slate-400 mt-1 line-clamp-2 leading-tight">Aset abadi berkelanjutan & pengganti ibadah puasa.</p>
                        </a>
                    </div>

                    <a href="/PeduliUmat/views/donasi.php" class="block w-full text-center py-3 rounded-xl bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold shadow-md shadow-brand-600/20 transition-colors">
                        Lihat Semua Opsi
                    </a>
                </div>
            </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
            <div>
                ...
                <h2 class="text-3xl font-extrabold text-slate-900 dark:text-white mt-1">Program PeduliUmat</h2>
            </div>
            <a href="/PeduliUmat/views/program.php" class="mt-4 md:mt-0 inline-flex items-center gap-2 font-bold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300">
                ...
                ?>
                    <div class="bg-slate-50 dark:bg-slate-800/60 border border-slate-100 dark:border-slate-700/60 rounded-3xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 flex flex-col group">
                        <div class="relative h-48 bg-slate-200 dark:bg-slate-700 overflow-hidden">
                            <img src="<?= htmlspecialchars($campaign['image'] ?? '/PeduliUmat/assets/uploads/campaigns/default.jpg'); ?>" alt="<?= htmlspecialchars($campaign['title']); ?>" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                        ...
                        <div class="p-6 flex-1 flex flex-col justify-between space-y-4">
                            ...
                            <a href="/PeduliUmat/views/donasi.php?campaign_id=<?= $campaign['id']; ?>" class="block w-full py-3 text-center rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm transition-colors shadow-md shadow-brand-500/10">
                                Donasi Sekarang
                            </a>
                        </div>
                    </div>
...
<section class="py-20 bg-white dark:bg-slate-900 transition-colors duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
            ...
            <a href="/PeduliUmat/views/berita.php" class="mt-4 md:mt-0 inline-flex items-center gap-2 font-bold text-brand-600 dark:text-brand-400 hover:text-brand-700 dark:hover:text-brand-300">
                ...
                <?php foreach ($articles as $article): ?>
                    <article class="bg-slate-50 dark:bg-slate-800/60 rounded-3xl border border-slate-100 dark:border-slate-700/60 overflow-hidden shadow-sm hover:shadow-md transition-all flex flex-col">
                        <div class="h-48 bg-slate-200 dark:bg-slate-700 overflow-hidden">
                            <img src="<?= htmlspecialchars($article['image'] ?? '/PeduliUmat/assets/uploads/articles/default.jpg'); ?>" alt="<?= htmlspecialchars($article['title']); ?>" class="w-full h-full object-cover">
                        </div>
                        <div class="p-6 flex-1 flex flex-col justify-between space-y-4">
                            <div>
                                ...
                                <h3 class="font-bold text-slate-900 dark:text-white text-lg mt-1 line-clamp-2 hover:text-brand-600 dark:hover:text-brand-400 transition-colors">
                                    <a href="/PeduliUmat/views/berita.php?slug=<?= $article['slug']; ?>"><?= htmlspecialchars($article['title']); ?></a>
                                </h3>
                            ...
                            <div class="pt-4 border-t border-slate-100 dark:border-slate-700/60 flex justify-between items-center text-xs font-semibold text-slate-500 dark:text-slate-400">
                                <span>Penulis: <?= htmlspecialchars($article['author_name']); ?></span>
                                <a href="/PeduliUmat/views/berita.php?slug=<?= $article['slug']; ?>" class="text-brand-600 dark:text-brand-400 font-bold hover:underline">Baca Selengkapnya</a>
                            </div>
                        </div>
                    </article>
                <?php endforeach; ?>
