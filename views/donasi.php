...
<div class="bg-slate-50 dark:bg-slate-950 py-12 lg:py-16 min-h-screen transition-colors duration-300">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php if ($success_donation): ?>
            <div class="bg-white dark:bg-slate-900 p-8 sm:p-12 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-xl dark:shadow-slate-900/40 text-center space-y-6 transition-colors">
                <div class="w-20 h-20 bg-emerald-100 dark:bg-emerald-950/60 text-brand-600 dark:text-brand-400 rounded-full flex items-center justify-center mx-auto text-4xl shadow-inner animate-bounce">
                    <i class="fa-solid fa-circle-check"></i>
                </div>
                <div class="space-y-2">
                    <h2 class="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white">Niat Baik Anda telah Terkirim!</h2>
                    <p class="text-slate-500 dark:text-slate-400 text-sm">Terima kasih telah menitipkan amanah donasi melalui SahabatPeduli.</p>
                </div>

                <div class="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-2xl border border-slate-200/60 dark:border-slate-700/60 max-w-md mx-auto text-left space-y-3 text-sm">
                    <div class="flex justify-between border-b border-slate-200/60 dark:border-slate-700/60 pb-2">
                        ...
                        <span class="font-mono font-bold text-slate-900 dark:text-slate-100"><?= $success_donation['order_id']; ?></span>
                    </div>
                    <div class="flex justify-between border-b border-slate-200/60 dark:border-slate-700/60 pb-2">
                        <span class="text-slate-500 dark:text-slate-400">Atas Nama:</span>
                        <span class="font-bold text-slate-900 dark:text-slate-100"><?= htmlspecialchars($success_donation['display_name']); ?></span>
                    </div>
                    <div class="flex justify-between border-b border-slate-200/60 dark:border-slate-700/60 pb-2">
                        <span class="text-slate-500 dark:text-slate-400">Kategori:</span>
                        <span class="font-bold text-brand-600 dark:text-brand-400 uppercase"><?= $success_donation['type']; ?></span>
                    </div>
                    <div class="flex justify-between pt-1 text-base">
                        <span class="font-bold text-slate-900 dark:text-white">Total Dibayar:</span>
                        <span class="font-black text-brand-600 dark:text-brand-400">Rp <?= number_format($success_donation['amount'], 0, ',', '.'); ?></span>
                    </div>
                </div>

                <div class="pt-4 flex flex-col sm:flex-row gap-4 justify-center">
                    <a href="/SahabatPeduli/index.php" class="px-8 py-3.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold text-sm shadow-md shadow-brand-500/20 transition-all">
                        Kembali ke Beranda
                    </a>
                    <a href="/SahabatPeduli/views/penyaluran.php" class="px-8 py-3.5 rounded-xl bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 font-bold text-sm transition-all">
        ...
        <?php else: ?>
            ...
            <div class="text-center max-w-xl mx-auto mb-10">
                ...
                <p class="text-slate-500 dark:text-slate-400 text-sm mt-2">Pilih kategori penyerahan dan nominal yang ingin Anda salurkan.</p>
            </div>

            <?php if ($error): ?>
                <div class="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800 text-red-700 dark:text-red-300 px-4 py-3 rounded-2xl text-xs mb-6">
