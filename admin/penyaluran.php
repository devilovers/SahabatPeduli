    ...
    <main class="flex-1 bg-white dark:bg-slate-950 py-8 px-4 sm:px-8 lg:px-12 overflow-y-auto space-y-8 sm:space-y-10 transition-colors duration-300">
        ...
        <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1">Input lokasi geografis dan detail laporan penyaluran dana donasi ke masyarakat.</p>
            </div>
                ...
                <table class="w-full text-left border-collapse min-w-[600px]">
                    <thead>
                        <tr class="bg-white dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800 text-[11px] font-black text-slate-400 dark:text-slate-400 uppercase tracking-wider">
                            ...
                            <th class="py-4 px-6">Dana Disalurkan</th>
                            <th class="py-4 px-6">Penerima Manfaat</th>
                            <th class="py-4 px-6">Koordinat (Lat, Long)</th>
                            ...
                            <?php foreach ($distributions as $dist): ?>
                                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                                    <td class="py-4 px-6 font-bold text-slate-900 dark:text-white">
                                        ...
                                        <span class="block text-[11px] text-slate-400 dark:text-slate-500 font-normal line-clamp-1 mt-0.5"><?= htmlspecialchars($dist['description']); ?></span>
                                    </td>
                                    ...
                                    <td class="py-4 px-6 font-semibold text-slate-700 dark:text-slate-300">
                                        <i class="fa-solid fa-users text-slate-400 dark:text-slate-500 mr-1"></i> <?= number_format($dist['beneficiaries_count'], 0, ',', '.'); ?> Orang
                                    </td>
                                    <td class="py-4 px-6 font-mono text-slate-500 dark:text-slate-400 text-[11px]">
...
<div id="modalCreateDist" class="fixed inset-0 bg-slate-900/50 dark:bg-slate-950/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden">
    <div class="bg-white dark:bg-slate-900 rounded-3xl max-w-lg w-full p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl border border-slate-100 dark:border-slate-800">
        <div class="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-4">
            <div>
                <h3 class="text-lg font-black text-slate-900 dark:text-white">Tambah Titik Penyaluran Peta</h3>
                <p class="text-xs text-slate-400 dark:text-slate-500">Tentukan lokasi geografis penerima bantuan.</p>
            </div>
        ...
        <form method="POST" action="" class="space-y-4 text-xs font-semibold">
            ...
            <div>
                <label class="block text-slate-700 dark:text-slate-300 mb-1">Nama Lokasi / Penerima</label>
                <input type="text" name="location_name" required class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none transition-all" placeholder="Contoh: Panti Asuhan Harapan, Banjarmasin Barat">
            </div>

            <div>
                <label class="block text-slate-700 dark:text-slate-300 mb-1">Kategori Bantuan</label>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label class="block text-slate-700 dark:text-slate-300 mb-1">Nominal Disalurkan (Rp)</label>
            ...
            <div>
                <label class="block text-slate-700 dark:text-slate-300 mb-1">Deskripsi Kegiatan Penyaluran</label>
                <textarea name="description" rows="3" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:outline-none transition-all" placeholder="Penyerahan bantuan sembako dan perbaikan sarana belajar..."></textarea>
            </div>
            ...
            <div class="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-slate-800">
                ...
                <button type="submit" class="px-5 py-2.5 rounded-xl bg-brand-600 hover:bg-brand-700 text-white font-bold transition-all shadow-md shadow-brand-500/20 active:scale-95">Simpan Titik</button>
            </div>
        </form>
    </div>
</div>
